...
description: Turn approved One Horizon Ink content ideas into reviewable drafts. Use when Codex is asked to process planned `[Ink Idea]` records, run the matching writer workflow or generic channel writer, update Blog initiatives in place from `[Ink Idea] [Blog]` to `[Ink Draft] [Blog]`, and leave draft content in One Horizon for human review. Do not use this skill for ideation, direct publishing, corpus storage, or automation setup.
---

# Content Creation Runner

One Horizon is the review source of truth. For Blog, follow the Blog Lifecycle Contract in `../one-horizon-context-setup/references/ink-initiative-hierarchy.md`. This workflow does not publish, schedule, send, upload externally, or create local draft files.
...
1. Resolve the selected Ink profile with `../one-horizon-context-setup/references/ink-profile-contract.md`, then resolve workspace, author, and the `Ink - Blog`, `Ink - LinkedIn`, `Ink - Reddit`, and optional `Ink - Channel Content` parent initiatives inside that workspace.
2. Find `Planned` records titled `[Ink Idea] [Blog] ...`, `[Ink Idea] [LinkedIn] ...`, `[Ink Idea] [Reddit] ...`, or `[Ink Idea] [<generic channel>] ...`; use `search_tasks`, then `get_task_details`.
...
4. Parse the Content Idea Brief. Required to draft: channel, angle, audience, and enough thesis direction to avoid inventing the core argument. Treat missing `Proof needed`, `Risks`, and `Next workflow` as draft assumptions. If optional Content Program metadata exists, preserve `program_id`, `format_id`, `run_id`, and `campaign_id` in the draft handoff.
...
7. Run the channel writer:
   ...
   - Generic channels: `../channel-content-writer/SKILL.md`; produce channel-native drafts, scripts, prompts, asset briefs, or manual handoff bundles without publishing, scheduling, sending, uploading, recording, or editing externally.
8. Store the draft:
   ...
   - LinkedIn and Reddit: create one child initiative with `create_initiative`, using the matching `Ink - Channel` parent, title `[Ink Draft] [Channel] <idea title without [Ink Idea]>`, status `In Review`, and a description containing draft content, source idea, review notes, assumptions, optional Content Program metadata, and `Ink automation checkpoint: content-creation-runner <ISO timestamp>`.
   - Generic channels: create one child initiative under `Ink - Channel Content` when available, title `[Ink Draft] [Channel] <idea title without [Ink Idea]>`, status `In Review`, and a description containing the channel-native output or handoff bundle, asset requirements, manual steps, source idea, review notes, assumptions, optional Content Program metadata, and `Ink automation checkpoint: content-creation-runner <ISO timestamp>`.
9. For Blog, comment on the same initiative with a short draft summary and `Ink automation checkpoint: content-creation-runner <ISO timestamp>`. Do not mark the initiative `Completed`; it is now the review draft. For LinkedIn, Reddit, and generic channels, comment on the source idea with the draft initiative link, then mark the source idea `Completed`.
...
- Generic channel ideas must include a target channel and enough format direction for `channel-content-writer`. If the chosen channel requires external publishing, scheduling, design, email, video, audio, ad, or messaging tools, produce a handoff bundle instead of pretending Ink can execute that step.
...
Return processed ideas, updated Blog initiatives, created dedicated or generic channel draft initiatives, handoff bundles, ideas moved to `In Review` for human input, and failures.
