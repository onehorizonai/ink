...
4. Read `../channel-content-writer/references/channel-format-index.md` and `../channel-content-writer/references/channel-format-registry.csv` when the program targets a broad channel or format.
5. Clarify only the missing high-impact details with `references/program-interview.md`.
6. For new programs, ask the required storage question before writing files: should this be `local/private`, or `public/tracked` as part of the open-source repo? Do not infer the answer from defaults.
7. Decide storage from the user's answer:
...
8. Audit relevant existing corpus and program examples:
...
9. Create or update the program pack using the required layout, with `channels` for destinations and `routes` for production paths.
10. Run `python3 scripts/validate_program.py <program-dir>` when the pack is created or changed.
11. If One Horizon tools are available and the user wants tracking, create or update `[Ink Program] <name>` under `Ink - Programs`.
...
- Keep channel-wide and format-wide optimization in `../channel-content-writer/references/channel-format-index.md`, `channel-format-registry.csv`, `channels/*.md`, and `formats/*.md`. Programs should inherit those rules and add only program-specific cadence, consistency, assets, approval flow, and KPIs.
...
- `../channel-content-writer/references/channel-format-index.md`: channel and format inheritance model used by broad channels.
- `../channel-content-writer/references/channel-format-registry.csv`: complete slug coverage registry.
