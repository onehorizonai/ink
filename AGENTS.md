...
- Keep unpublished drafts under the selected Ink profile's local content roots. Legacy paths are `content/linkedin/drafts/`, `content/reddit/drafts/`, and `content/blog/drafts/` only when no profile config exists. Generic channel drafts belong under the selected profile's `contentRoots.channels` or `.local/content/<profile-id>/channels/`.
