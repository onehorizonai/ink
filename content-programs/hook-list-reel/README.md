...
```mermaid
flowchart TD
    A["Choose one Reel topic and audience"] --> B["Draft hook variants"]
    B --> C["Select one honest overlay hook"]
    C --> D["Create video asset brief"]
    D --> E["Write keyword-rich list description"]
    E --> F["Choose exactly 5 hashtags"]
    F --> G["Run manual Reels QA"]
    G --> H["Publish or schedule externally"]
    H --> I["Log performance and learnings"]
```
